  ...
  const fetchApiData = async (page = 1) => {
    try {
      const response = await fetch(
        `https://phimapi.com/v1/api/danh-sach/hoat-hinh?limit=32&page=${page}`
      );
      if (!response.ok) {
      ...
      if (params && params.pagination) {
        ...
        console.log("Pagination data not found in API response:", data);
      }
    } catch (error) {
      console.log("Không thể kết nối đến API thể loại", error);
    }
  };
      ...
      <div className="relative">
        ...
        >
          <div className="w-[90%] mx-auto mb-4 sm:mb-10 md:mb-20">
            {movie && (
              <h2 className="text-white md:text-6xl text-3xl mb-5">
                {movie.titlePage}
              </h2>
            )}
...
export default ListFilmHoatHinh;
